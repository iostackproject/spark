...
/**
 * Implementation storing data in Swift as array off blocks.
...
public class SwiftBlockFileSystem extends FileSystem {
  ...
  public static final String BSWIFT = "bswift";
  ...
  public void initialize(URI fsuri, Configuration conf) throws IOException {
    ...
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.initialize uri = "+fsuri);       // TODO TODO log
    setConf(conf);
    ...
    //this.workingDir = new Path("/user", System.getProperty("user.name")).makeQualified(uri, new Path(System.getProperty("user.name")));
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Initializing SwiftBlockFileSystem against URI " + uri
        + " and working dir " + workingDir);
    }
    store.initialize(uri, conf);
    LOG.debug("SwiftBlockFileSystem initialized with uri "+uri);
  }
  ...
  @Override
  public URI getUri() {
    LOG.info("CAMAMILLA SwiftBlockFileSystem.getUri "+uri);      // TODO TODO log
    return uri;
  }
  ...
  public boolean mkdirs(Path path, FsPermission permission)
          throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.mkdir path = "+path);       // TODO TODO log
  ...
  public boolean isFile(Path path) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.isFile path = "+path);       // TODO TODO log
  ...
  public FileStatus[] listStatus(Path path) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.listStatus path = "+path);       // TODO TODO log
    Path absolutePath = makeAbsolute(path);
    ...
    if (inode.isFile()) {
      return new FileStatus[]{getFileStatus(
              path.makeQualified(uri, workingDir), inode)
      };
    }
  ...
  private Path makeAbsolute(Path path) {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.makeAbsolute path = "+path);       // TODO TODO log
    if (path.isAbsolute()) {
      return path;
    }
  ...
  private INode checkFile(Path path) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.checkFile path = "+path);       // TODO TODO log
    INode inode = store.retrieveINode(makeAbsolute(path));
    if (inode == null) {
  ...
  public FSDataOutputStream create(Path file, FsPermission permission,
                                   ...
                                   Progressable progress)
          throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.create path = "+file);       // TODO TODO log
  ...
  public FSDataInputStream open(Path path, int bufferSize) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.open path = "+path);       // TODO TODO log
  ...
  public boolean rename(Path src, Path dst) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.rename path = "+src);       // TODO TODO log
  ...
  private boolean renameRecursive(Path src, Path dst) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.renameRecursive path = "+src);       // TODO TODO log
  ...
  public boolean delete(Path path, boolean recursive) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.delete path = "+path);       // TODO TODO log
    Path absolutePath = makeAbsolute(path);
    INode inode = store.retrieveINode(absolutePath);
    if (inode == null) {
      return false;
  ...
  public boolean delete(Path path) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.delete deprecated path = "+path);       // TODO TODO log
  ...
  public FileStatus getFileStatus(Path f) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getFileStatus path = "+f);       // TODO TODO log
  ...
  public BlockLocation[] getFileBlockLocations(FileStatus file,
                                               long start, long len)
          throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getFileBlockLocations path = "+file+" start="+start+" length="+len);       // TODO TODO log
  ...
  public long getDefaultBlockSize(Path path) {
    //64 mb
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getDefaultBlockSize sempre 64MB path = "+path);       // TODO TODO log
    return 64 * 1024 * 1024;
  }

  private FileStatus getFileStatus(Path f, INode inode) throws IOException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getFileStatus 2 path = "+f);       // TODO TODO log
  ...
  private long getFileLength(INode inode) {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getFileLength inode = "+inode);       // TODO TODO log
    if (!inode.isDirectory()) {
      long length = 0L;
      for (Block block : inode.getBlocks()) {
        length += block.getLength();
  ...
  private long getBlockSize(INode inode) {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getBlockSize inode = "+inode);       // TODO TODO log
  ...
  private Path getCorrectSwiftPath(Path path) throws URISyntaxException {
    LOG.info("CAMAMILLA BLOCK SwiftBlockFileSystem.getCorrectSwiftPath path = "+path);       // TODO TODO log
