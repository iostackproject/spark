...
public class SwiftNativeFileSystemStore {
  ...
  public void uploadFile(Path path, InputStream inputStream, long length)
          throws IOException {
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.uploadFile path = "+path);      // TODO TODO log

      // TODO TODO clientstorlets
  ...
  public void uploadFilePart(Path path, int partNumber,
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.uploadFilePart path = "+path);      // TODO TODO log
  ...
  public void createManifestForPartUpload(Path path) throws IOException {
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.createManifestForPartUpload path = "+path);      // TODO TODO log
  ...
  public SwiftFileStatus getObjectMetadata(Path path, boolean newest)
    ...
    // execute HEAD HTTP
    final Header[] headers = getObjectHeaders(path, newest);
    ...
    for (Header header : headers) {
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.getObjectMetadata path = " + path + " header = {"+lastModified+"}");      // TODO TODO log
      ...
      if (SwiftProtocolConstants.HEADER_LAST_MODIFIED.equals(headerName)) {
        ...
        try {
          ...
          // Es possible que aixo doni problemes, de fet en la versio 2.5.1 vaig haver de canviar el codi de dalt pel daqui sota
          //LOG.error("CAMAMILLA SwiftNativeFileSystemStore.getObjectMetadata Failed to parse PATTERN=" + PATTERN + " getvalue={" + header.getValue() + "} \nCAMAMILLA header={" + header.toString() + "}");      // TODO TODO log
          //throw new SwiftException("Failed to parse " + header.toString(), e);
          lastModified = 0;   // TODO TODO error
        }
      }
    }
    if (lastModified == 0) {
  ...
  private Header[] stat(SwiftObjectPath objectPath, boolean newest) throws
    ...
    Header[] headers;
    if (newest) {
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.stat new path = "+objectPath);      // TODO TODO log
      headers = swiftRestClient.headRequest("getObjectMetadata-newest",
                                            objectPath, SwiftRestClient.NEWEST);
    } else {
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.stat path = "+objectPath);      // TODO TODO log
  ...
  private List<FileStatus> listDirectory(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.listDirectory path = "+path);      // TODO TODO log
    try {
      ...
      if (e.getStatusCode() == HttpStatus.SC_NO_CONTENT) {
        ...
        if (SwiftUtils.isRootDir(path)) {
          ...
          LOG.info("CAMAMILLA SwiftNativeFileSystemStore.listDirectory SwiftInvalidResponseException is not root dir stat " + stat);      // TODO TODO log
        ...
        LOG.error("CAMAMILLA SwiftNativeFileSystemStore.listDirectory unknow status code "+e.getMessage());      // TODO TODO log
        throw e;
      }
    }
    ...
    if (fileStatusList.isEmpty()) {
      ...
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.listDirectory path = "+path+" is empty, metadata = {"+objectMetadata+"}");      // TODO TODO log
    ...
    for (SwiftObjectFileStatus status : fileStatusList) {
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.listDirectory path = "+path+" filestatus = "+status);      // TODO TODO log
  ...
  public FileStatus[] listSubPaths(Path path,
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.listSubPaths path = "+path);      // TODO TODO log
  ...
  private void innerCreateDirectory(SwiftObjectPath swiftObjectPath)
          throws IOException {
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.innerCreateDirectory path = "+swiftObjectPath);      // TODO TODO log
  ...
  public List<URI> getObjectLocation(Path path) throws IOException {
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.getObjectLocation path = "+path);      // TODO TODO log
  ...
  public boolean deleteObject(Path path) throws IOException {
    ...
    if (!SwiftUtils.isRootDir(swiftObjectPath)) {
      LOG.info("CAMAMILLA SwiftNativeFileSystemStore.deleteObject path = "+path);      // TODO TODO log
  ...
  public boolean objectExists(SwiftObjectPath path) throws IOException {
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.objectExists path = "+path);      // TODO TODO log
    try {
  ...
  public void rename(Path src, Path dst)
    ...
    if (srcIsFile) {
      ...
      if (childCount == 0) {
        ...
        LOG.info("CAMAMILLA SwiftNativeFileSystemStore.rename path = "+srcObject);      // TODO TODO log
  ...
  public void copy(Path srcKey, Path dstKey) throws IOException {
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.copy src = "+srcKey+" dst = "+dstKey);      // TODO TODO log
  ...
  private void copyThenDeleteObject(SwiftObjectPath srcObject,
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.copyThenDeleteObject src = "+srcObject+" dst = "+destObject);      // TODO TODO log
  ...
  private void copyObject(SwiftObjectPath srcObject,
    ...
    LOG.info("CAMAMILLA SwiftNativeFileSystemStore.copyObject src = "+srcObject+" dst = "+destObject);      // TODO TODO log
    //do the copy
