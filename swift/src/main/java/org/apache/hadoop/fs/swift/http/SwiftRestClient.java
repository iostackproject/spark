...
public final class SwiftRestClient {
  ...
  * the name of a proxy host (can be null, in which case there is no proxy)
  ...
  private SwiftRestClient(URI filesystemURI,
    ...
    storlet_name = conf.get("storlet.jar");
    storlet_params = conf.get("storlet.params");
  ...
  public HttpBodyContent getData(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.getData A path = "+path+" offset="+offset+" length="+length);      // TODO TODO log
    if (offset < 0) {
  ...
  public long getContentLength(URI uri) throws IOException {
    ...
    LOG.info("CAMAMILLA SwiftRestClient.getContentLength uri = " + uri);      // TODO TODO log
  ...
  public HttpBodyContent getData(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.getData C path = " + path +" amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log

    if (storlet_name != null && !storlet_name.equalsIgnoreCase("")) {
      LOG.info("CAMAMILLA SwiftRestClient.getData C path = " + path +" request with storlet");      // TODO TODO log

      URI url = pathToURI(path);
      URI uri = pathToURI(path);

      try {
        String withParams = url.toString()+"?"+storlet_params;
        uri = new URI(withParams);
      } catch (URISyntaxException e) {
        LOG.info("CAMAMILLA SwiftRestClient.getData C path = " + path +" request with storlet error {"+e.getMessage()+"}");      // TODO TODO log
      }
      return doGet(uri,
        addStorletsHeaders(requestHeaders));
    } else {
      LOG.info("CAMAMILLA SwiftRestClient.getData C path = " + path +" request without storlet");      // TODO TODO log
  ...
  public byte[] getObjectLocation(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.getObjectLocation C path = " + path +" amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  private URI pathToObjectLocation(SwiftObjectPath path) throws SwiftException {
    LOG.info("CAMAMILLA SwiftRestClient.pathToObjectLocation path = " + path);      // TODO TODO log
    URI uri;
  ...
  public byte[] findObjectsByPrefix(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.findObjectsByPrefix path = " + path +" amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  public byte[] listDeepObjectsInDirectory(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.listDeepObjectsInDirectory path = " + path +" amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  private byte[] findObjects(String location, final Header[] requestHeaders) throws
    ...
    LOG.info("CAMAMILLA SwiftRestClient.findObjects location = " + location +" amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  public boolean copyObject(SwiftObjectPath src, final SwiftObjectPath dst,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.copyObject src = " + src + " dest = " + dst + " amb headers = {"+headersToString(headers)+"}");      // TODO TODO log
  ...
  public void upload(SwiftObjectPath path,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.upload path = " + path + " amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  public boolean delete(SwiftObjectPath path, final Header... requestHeaders) throws IOException {
    ...
    LOG.info("CAMAMILLA SwiftRestClient.delete path = " + path + " amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  public Header[] headRequest(String reason,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.headRequest reason = " + reason + " path = " + path + " amb headers = {" + headersToString(requestHeaders) + "}");      // TODO TODO log
  ...
  public int putRequest(SwiftObjectPath path, final Header... requestHeaders)
    ...
    LOG.info("CAMAMILLA SwiftRestClient.putRequest path = " + path + " amb headers = {"+headersToString(requestHeaders)+"}");      // TODO TODO log
  ...
  public AccessToken authenticate() throws IOException {
    ...
    LOG.info("CAMAMILLA SwiftRestClient.authenticate");      // TODO TODO log
  ...
  private class AuthenticationPost extends AuthMethodProcessor<AccessToken> {
    ...
    public AccessToken extractResult(AuthPostMethod method) throws IOException {
      LOG.info("CAMAMILLA SwiftRestClient.extractResult");      // TODO TODO log
  ...
  private synchronized void createDefaultContainer() throws IOException {
    LOG.info("CAMAMILLA SwiftRestClient.createDefaultContainer");      // TODO TODO log
  ...
  public void createContainer(String containerName) throws IOException {
    LOG.info("CAMAMILLA SwiftRestClient.createContainer name = "+containerName);      // TODO TODO log
  ...
  private <M extends HttpMethod, R> R perform(URI uri,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.perform uri = " + uri);      // TODO TODO log
  ...
  private <M extends HttpMethod, R> R perform(String reason,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.perform headers = {" + headersToString(method.getRequestHeaders()) + "} uri ={" + method.getURI() + "}");      // TODO TODO log
  ...
  private <M extends HttpMethod> Header[] addStorletsHeaders(Header... headers) {

    int size = headers.length;
    Header[] newestHeaders = new Header[size+1];

    for (int i = 0; i< size; i++) {
      newestHeaders[i] = headers[i];
    }
    newestHeaders[size] = new Header("X-Run-Storlet", storlet_name);
    //newestHeaders[size+1] = new Header("X-Storlet-Generate-Log", "True");
    LOG.info("CAMAMILLA SwiftRestClient.addStorletsHeaders");      // TODO TODO log
    return newestHeaders;
  }
  ...
  public static SwiftRestClient getInstance(URI filesystemURI,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.getInstance uri = " + filesystemURI);      // TODO TODO log
  ...
  private <M extends HttpMethod> int exec(M method) throws IOException {
    LOG.info("CAMAMILLA SwiftRestClient.exec method = " + method.getName());      // TODO TODO log

    final HttpClient client = new HttpClient();
    if (proxyHost != null) {
      LOG.info("CAMAMILLA SwiftRestClient.exec method A proxyHost="+proxyHost);      // TODO TODO log
    ...
    if ((statusCode == HttpStatus.SC_UNAUTHORIZED
      ...
      LOG.info("CAMAMILLA SwiftRestClient.exec method B Operation failed with status " + method.getStatusCode() + " attempting keystone auth");      // TODO TODO log
      if (LOG.isDebugEnabled()) {
    ...
    if (statusCode == HttpStatus.SC_UNAUTHORIZED ) {
      ...
      statusCode = execWithDebugOutput(method, client);
      LOG.info("CAMAMILLA SwiftRestClient.exec method C unauthed -or the auth uri rejected it statuscode="+statusCode);      // TODO TODO log
    }
    return statusCode;
  }
  ...
  private <M extends HttpMethod> int execWithDebugOutput(M method,
    ...
    LOG.info("CAMAMILLA SwiftRestClient.execWithDebugOutput buidler = "+builder+" "+headersToString(method.getRequestHeaders()));       // TODO TODO log
  ...
  private String headersToString(Header... headers) {
    ...
    for (Header h : headers) {
      if (!h.getName().equalsIgnoreCase("X-Auth-Token")) sheaders += h +" | ";
    }
